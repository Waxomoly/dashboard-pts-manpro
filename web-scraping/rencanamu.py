...
class RencanamuScraper:
    ...
    def is_javascript_content(self, text):
        ...
        # Pattern JavaScript/JSON yang umum
        js_patterns = [
            r'function\s*\(',
            r'var\s+\w+\s*=',
            r'const\s+\w+\s*=',
            ...
            r'return\s+\w+',
            r'\{"".*"":\s*"".*""\}',  # JSON format
            r'pp_id|pp_for|pp_post',  # Specific to your example
            r'ppc_id|ppa_id',
            r'owl.*Carousel',
            r'getElementById',
            r'querySelector',
    ...
    def get_clean_text(self, soup):
        ...
        # Clone soup agar tidak merusak original
        ...
        # Split menjadi lines dan filter
        lines = text.split('\n')
        clean_lines = []
        ...
        for line in lines:
            ...
            # Skip lines yang terdeteksi sebagai JavaScript
            if self.is_javascript_content(line):
                continue
            
            # Skip lines yang terlalu pendek (< 3 karakter)
            if len(line) < 3:
                continue
    ...
    def extract_biaya_from_text(self, text, soup):
        ...
        # Clean text dari karakter aneh
        text = text.replace('\xa0', ' ')
        
        # Cari data di dalam div dengan class tertentu (TANPA SCRIPT!)
        ...
        for section in biaya_sections:
            # Hapus script dari section sebelum ambil text
            ...
            # Skip jika terdeteksi JavaScript
            if not self.is_javascript_content(section_text):
                combined_text += " " + section_text
        
        # TIDAK AMBIL DARI SCRIPT TAGS!
        # Script tags sudah dihapus dari soup sebelumnya
        ...
        # Coba cari pattern angka saja untuk biaya (fallback)
    ...
    def get_university_details(self, kampus_url):
        ...
        try:
            ...
            # BERSIHKAN SOUP DARI SCRIPT DULU!
            clean_soup = self.clean_soup_from_scripts(soup)
            
            # Nama kampus - Multiple fallback
            nama = "-"
            ...
            # Ekstrak biaya - gunakan clean_soup!
    ...
    def export_summary_report_single_file(self, csv_filename, output_filename='scraping_summary.txt'):
        ...
        try:
            ...
            with open(output_filename, 'w', encoding='utf-8') as f:
                ...
                # Hitung per kampus (bukan per baris)
                ...
                # Top 10 kampus dengan prodi terbanyak
                f.write("\n" + "="*60 + "\n")
    ...
    def export_summary_report(self, output_filename='scraping_summary.txt'):
        ...
        # Panggil fungsi baru dengan file default
    ...
    def scrape_all(self, start_prov=1, end_prov=34):
        ...
        # arahkan ke folder csv_result di luar web-scraping
        ...
        for prov_id in provinces:
            ...
            for idx, uni in enumerate(universities, 1):
                ...
                # Rename akreditasi kampus
                details['akreditasi_kampus'] = details.pop('akreditasi')
                
                if programs:
                    # Tulis per prodi (1 baris = 1 prodi dengan info kampus lengkap)
                    for prog in programs:
                        row = {**details, **prog}  # Gabung dict kampus + prodi
                        writer.writerow(row)
                        total_prodi += 1
                else:
                    # Kalau tidak ada prodi, tetap tulis info kampus
    ...
    def save_to_csv(self, kampus_data, jurusan_data, kampus_filename='kampus_data_single.csv'):
        ...
        if jurusan_data:
            # Gabung data kampus + setiap prodi
            for prodi in jurusan_data:
                row = {**kampus_data, **prodi}
                all_rows.append(row)
        else:
            # Kalau tidak ada prodi, tetap simpan info kampus
